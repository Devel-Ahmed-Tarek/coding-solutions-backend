@extends('layouts.dashboard')

@section('title', 'عرض المشروع')

@section('content')
<div class="mb-8 flex justify-between items-start">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">{{ $project->title }}</h1>
        <p class="text-gray-500 mt-1">عرض تفاصيل المشروع</p>
    </div>
    <div class="flex gap-2">
        <a href="{{ route('dashboard.projects.edit', $project) }}" class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition text-sm">تعديل</a>
        <a href="{{ route('dashboard.projects.index') }}" class="px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 transition text-sm">رجوع</a>
    </div>
</div>

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 max-w-2xl">
    <dl class="space-y-4">
        <div>
            <dt class="text-sm font-medium text-gray-500">العنوان</dt>
            <dd class="mt-1 text-gray-800">{{ $project->title }}</dd>
        </div>
        <div>
            <dt class="text-sm font-medium text-gray-500">الوصف</dt>
            <dd class="mt-1 text-gray-800">{{ $project->description ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-sm font-medium text-gray-500">الفئة</dt>
            <dd class="mt-1 text-gray-800">{{ $project->category ?? '—' }}</dd>
        </div>
        <div>
            <dt class="text-sm font-medium text-gray-500">الرابط</dt>
            <dd class="mt-1 text-gray-800">
                @if($project->link)
                    <a href="{{ $project->link }}" target="_blank" rel="noopener" class="text-blue-600 hover:underline">{{ $project->link }}</a>
                @else
                    —
                @endif
            </dd>
        </div>
        <div>
            <dt class="text-sm font-medium text-gray-500">التقنيات</dt>
            <dd class="mt-1 text-gray-800">{{ $project->technologies->pluck('name')->join(', ') ?: '—' }}</dd>
        </div>
        <div>
            <dt class="text-sm font-medium text-gray-500">الحالة</dt>
            <dd class="mt-1">
                <span class="px-3 py-1 rounded-full text-xs {{ $project->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                    {{ $project->is_active ? 'نشط' : 'غير نشط' }}
                </span>
            </dd>
        </div>
        @if($project->images->isNotEmpty())
        <div>
            <dt class="text-sm font-medium text-gray-500 mb-2">الصور</dt>
            <dd class="flex flex-wrap gap-2">
                @foreach($project->images as $img)
                <img src="{{ asset('storage/' . $img->path) }}" alt="" class="w-20 h-20 object-cover rounded-lg border border-gray-200">
                @endforeach
            </dd>
        </div>
        @endif
    </dl>
</div>
@endsection
