    ...
    <form action="{{ route('dashboard.articles.store') }}" method="POST">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="5" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">{{ old('description') }}</textarea>
                @error('description')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">الرابط (slug) - يترك فارغاً للتوليد التلقائي</label>
                <input type="text" name="slug" value="{{ old('slug') }}" placeholder="article-slug" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">الفئة (مثل: تطوير الويب، تصميم)</label>
